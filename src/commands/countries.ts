...
export abstract class Countries {
  ...
  @Slash("countries", {
    description: "Pagination for supported Ikea countries to query",
  })
  async countries(interaction: CommandInteraction): Promise<void> {
    ...
    for (var i = 0; i < supported_countries.length; i += 2) {
      ...
      fields.push(
        {
          name: "Name",
          value: `:flag_${supported_countries[i].code}: ${supported_countries[i].name}`,
          inline: true,
        },
        { name: "Code", value: supported_countries[i].code, inline: true },
        {
          name: "Stores",
          value: supported_countries[i].stores.length.toString() || "0",
          inline: true,
        }
      );
      ...
      if (supported_countries[i + 1] !== undefined) {
        ...
        fields.push(
          ...
          {
            name: "Name",
            value: `:flag_${supported_countries[i + 1].code}: ${
          ...
          {
            name: "Code",
            value: supported_countries[i + 1].code,
            inline: true,
          },
          {
            name: "Stores",
            value: supported_countries[i + 1].stores.length.toString() || "0",
            inline: true,
          }
        );
      }
