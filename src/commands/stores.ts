...
@SlashGroup({
  name: "stores",
  description: "Display information related to stores.",
})
@SlashGroup("stores")
export abstract class Stores {
  ...
  async store(
    @SlashOption("store_id", {
      description: "Store id to list the respective store information for",
    ...
    const store = supported_stores.find(
      (store: Store) => store.buCode == store_id
    );
    ...
    const store_country = supported_countries.find(
      (country: Country) => country.code == store.countryCode
    );
      ...
      embeds: [
        new EmbedBuilder()
          ...
          .addFields(
            ...
            {
              name: "Country",
              value: `:flag_${store_country?.code}: ${store_country?.name}`,
              inline: true,
            },
            {
              name: "Latitude",
              value: store.coordinates[1].toString(),
              inline: true,
            },
            {
              name: "Longitude",
              value: store.coordinates[0].toString(),
              inline: true,
            },
            { name: "\u200B", value: "\u200B" },
            {
              name: "Google Maps",
              value: `https://www.google.com/maps/search/?api=1&query=Ikea+${store.name}`,
              inline: true,
            }
          )
          .setURL(
            `https://www.google.com/maps/search/?api=1&query=Ikea+${store.name}`
          ),
      ],
    });
  }
  ...
  async country(
    @SlashOption("country_code", {
      description: "Country code to display all stores for the given country",
      type: ApplicationCommandOptionType.String,
      required: true,
    })
    country_code: string,
    ...
    if (country == undefined) {
      // Reply with an error message
      await interaction.reply(
        "Oops... That country wasn't found. Try `/countries` to view a list of supported countried and thier respective country codes."
      );
    ...
    for (var i = 0; i < country.stores.length; i += 2) {
      ...
      pages.push(
        new EmbedBuilder()
          .setFooter({
            text: `Page ${Math.ceil(i / 2) + 1} of ${Math.ceil(
              country.stores.length / 2
            )}`,
          })
