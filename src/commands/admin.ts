...
export abstract class Admin {
	...
	async alertsChannel(@SlashOption("channel", { description: "Channel to send notifications to.", required: true }) channel: Channel, interaction: CommandInteraction): Promise<any> {
		// Send the data off to be added into the local DB
		await interaction.reply(`Stock notification messages will now be shared in ${channel.toString()}`);
