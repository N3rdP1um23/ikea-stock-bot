...
export abstract class Stock {
  ...
  @Slash()
  async store(
    @SlashOption("store_id", {
      description: "Store id to query against a single store",
      type: ApplicationCommandOptionType.String,
      required: true,
    })
    store_id: string,
    @SlashOption("article_number", {
      description: "article number (123.456.78/12345678 or s12345678)",
      type: ApplicationCommandOptionType.String,
      required: true,
    })
    ...
    if (!store) {
      ...
      await interaction.reply(
        'Oops... That store wasn\'t found. Try `/store country country_code:` (where `country_code` is the countries respective "code" which can be found using the `/countries` command) to view a list of all stores for a given country.'
      );
    ...
    if (
      (article.toLowerCase().startsWith("s") && article.length != 9) ||
      (/^\d+$/.test(article) && article.length != 8)
    ) {
      // Reply with an error message
      await interaction.reply(
        "Oops... That article number is invalid. Please try again. (looks something like s12345678 or 123.456.78 found either on the page or in the URL)"
      );
    ...
    await interaction.reply(
      "Just processing the stock results now and will share them with you shortly!"
    );
    ...
    if (
      typeof item_stock == "object" &&
      item_stock.stock !== undefined &&
      item_stock.article !== undefined
    ) {
    ...
    var forecast_data: EmbedField[] = [];

    // Push a spacer to the forecast data
    forecast_data.push({ name: "\u200B", value: "\u200B", inline: true });
    ...
    if (item_stock.forecast.length > 0) {
      ...
      forecast_data.push({ name: "Forecast", value: "--------", inline: true });
      ...
      item_stock.forecast.forEach((forecast: any) => {
        ...
        forecast_data.push({
          ...
          value: forecast.stock.toString() || "0",
          inline: true,
        });
        ...
        forecast_data.push({
          ...
          value: forecast.date.toLocaleDateString(undefined, {
            year: "numeric",
            month: "long",
            day: "numeric",
          }),
        ...
        forecast_data.push({
          name: "Estimated Restock Probability",
          value: `${stock_status_icon[forecast.probability]} ${
      ...
      forecast_data.push({
        name: "Forecast",
        value: "No forecast data available currently.",
        inline: true,
      });
    }

    // Query to see if the user has already set a reminder for this store and article item
    const has_setup_reminder = Db.userHasReminder(
      interaction.user,
      store.buCode,
      article
    );
    ...
    await interaction.channel?.send({
      components: [
        new ActionRowBuilder<ButtonBuilder>().addComponents(
          new ButtonBuilder({
            customId: !has_setup_reminder
              ? "set-stock-reminder"
              : "unset-stock-reminder",
            label: !has_setup_reminder
              ? "Set Stock Reminder"
              : "Unset Stock Reminder",
            emoji: !has_setup_reminder ? "🔔" : "🔕",
            style: !has_setup_reminder
              ? ButtonStyle.Primary
              : ButtonStyle.Primary,
      ...
      embeds: [
        new EmbedBuilder()
          .setURL(
            `https://www.ikea.com/${country.code}/${country.language}/p/-${article}`
          )
          .setColor(stock_status_colours[item_stock.probability])
          .setTitle(
            `**Ikea :flag_${country.code}: ${country.name} - ${store.name} - ${article} Stock**`
          )
          .setFooter({
            text: `As of ${item_stock.createdAt.toLocaleDateString(undefined, {
              year: "numeric",
              month: "long",
              day: "numeric",
            })} @ ${item_stock.createdAt.toLocaleTimeString([], {
              hour12: false,
              hour: "2-digit",
              minute: "2-digit",
            })}`,
          ...
          .addFields(
            ...
            {
              name: "Current Stock",
              value: item_stock.stock.toString() || "0",
              inline: true,
            },
            {
              name: "Probability of Availability",
              value: `${stock_status_icon[item_stock.probability]} ${
                item_stock.probability
              }`,
              inline: true,
            },
            {
              name: "Estimated Restock Date",
              value: item_stock.restockDate
                ? item_stock.restockDate.toLocaleDateString(undefined, {
                    year: "numeric",
                    month: "long",
                    day: "numeric",
                  })
  ...
  @Slash()
  async country(
    @SlashOption("country_code", {
      description:
        "Country code to check stock of article for all stores in this country",
      type: ApplicationCommandOptionType.String,
      required: true,
    })
    country_code: string,
    ...
    if (!country) {
      // Reply with an error message
      await interaction.reply(
        "Oops... That country wasn't found. Try `/countries` to view a list of all countries and their respective country code."
      );
    ...
    if (Object.keys(country_store_stock).length <= 0) {
      // Reply with an error message
      await interaction.reply(
        "Oops... Unfortunately we're unable to display stock information for this article under this country as there's no stock information currently. Please try again later."
      );
    ...
    for (const store_buCode in country_store_stock) {
      ...
      const current_page =
        Object.keys(country_store_stock).indexOf(store_buCode);
      ...
      pages.push({
        ...
        embeds: [
          new EmbedBuilder()
            ...
            .setFooter({
              text: `Page ${Math.ceil(current_page + 1)} of ${Math.ceil(
                total_pages
              )}`,
            })
            .addFields(
              ...
              {
                name: "Current Stock",
                value: store_stock.stock.toString() || "0",
                inline: true,
              },
              {
                name: "Probability of Availability",
                value: `${stock_status_icon[store_stock.probability]} ${
                  store_stock.probability
                }`,
                inline: true,
              },
              {
                name: "Estimated Restock Date",
                value: store_stock.restockDate
                  ? store_stock.restockDate.toLocaleDateString(undefined, {
                      year: "numeric",
                      month: "long",
                      day: "numeric",
                    })
    ...
    await interaction.channel?.send(
      `<@${interaction.member?.user.id}> Your stock results are ready!`
    );
  ...
  @Slash()
  async stores(
    @SlashOption("store_ids", {
      description:
        "Comma separated list of store ids to check the article stock",
      type: ApplicationCommandOptionType.String,
      required: true,
    })
    store_ids: string,
    ...
    for (const store_id of store_ids.split(",")) {
      ...
      if (!/^\d+$/.test(store_id.trim()) || store == undefined) {
        // Reply with an error message
        await interaction.reply(
          `Oops... Unfortunately, the following store id is invalid or cannot be found (**${store_id}**). Try \`/store country country_code:\` (where \`country_code\` is the countries respective "code" which can be found using the \`/countries\` command) to view a list of all stores for a given country.`
        );
    ...
    if (Object.keys(stores_stock).length <= 0) {
      // Reply with an error message
      await interaction.reply(
        "Oops... Unfortunately we're unable to display stock information for this article under these stores as there's no stock information currently. Please try again later."
      );
  ...
  setStockReminder(interaction: ButtonInteraction) {
    ...
    const store_id = interaction.message.embeds[0].fields?.find(
      (field) => field.name == "Id"
    )?.value;
    const article = interaction.message.embeds[0].fields?.find(
      (field) => field.name == "Article"
    )?.value;

    // Handle adding the reminder
    const has_set_reminder = Db.userSetReminder(
      interaction.user,
      store_id,
      article,
      interaction.channel,
  ...
  unsetStockReminder(interaction: ButtonInteraction) {
    ...
    const has_unset_reminder = Db.userRemoveReminder(
      interaction.user,
      store_id,
      article,
      interaction.channel,
  ...
  static async handleReminders() {
    ...
    for (const reminder of reminders) {
      ...
      // Query for the articles stock
      let item_stock = await Stock.checkAvailability(
        store.buCode,
        reminder.ikea_article
      );
        ...
        if (item_stock.stock > 0) {
          client.channels.cache.get("772259249966809121").send({
            ...
            embeds: [
              new EmbedBuilder()
                .setURL(
                  `https://www.ikea.com/${country.code}/${country.language}/p/-${reminder.ikea_article}`
                )
                .setTitle(
                  `**Ikea :flag_${country.code}: ${country.name} - ${store.name} - ${reminder.ikea_article} Stock**`
                )
                .addFields(
                  { name: "Name", value: store.name, inline: true },
                  { name: "Id", value: store.buCode, inline: true },
